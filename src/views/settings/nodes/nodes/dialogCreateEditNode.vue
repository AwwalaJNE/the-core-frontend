...
export default {
    ...
    methods: {
        ...
        async getDataNode() {
            await axios
                .get(
                    this.URL.node + `?n=${this.listenNodeId}&sort_order=desc&limit=1000&page=1`,
                    this.Helper.header()
                )
                .then((res) => {
                    if (res.data.data.length > 0) {
                        ...
                        this.$store.dispatch(
                            'SET_NODE_DEFAULT_NODE_LINK_ID_ArrData',
        ...
        async getIpAddress() {
            await axios
                .get(this.URL.node_ip_address + `?n=${this.listenNodeId}`, this.Helper.header())
                .then((res) => {
                    if (res.data.data.length > 0) {
                        let uniqueIPs = {}
                        res.data.data.forEach((item) => {
                            if (item.ip_address !== null) {
                                let ipAddresses = item.ip_address
                                    .split(',')
            ...
            await axios
                ...
                .then((res) => {
                    if (res.data.data.length > 0) {
                        ...
                        this.$store.dispatch(
                            'SET_NODE_DEFAULT_NODE_ALTERNATE_ADDRESS_ID_ArrData',
                            arr.length > 0 ? arr : null
                        )
                    } else {
                        // this.openNotification('warn', null, 'Roles data is empty!', ' Please create a new role data')
        ...
        async getDataTariff() {
            await axios
                .get(
                    this.URL.tariff + `?n=${this.listenNodeId}&sort_order=desc&limit=1000&page=1`,
                    this.Helper.header()
                )
                .then((res) => {
                    if (res.data.data.length > 0) {
                        ...
                        this.$store.dispatch(
                            'SET_NODE_NODE_TARIFF_CODE_ArrData',
                            arr.length > 0 ? arr : null
                        )
                    } else {
                        // this.openNotification('warn', null, 'Roles data is empty!', ' Please create a new role data')
        ...
        async getDataCustomer() {
            await axios
                .get(
                    this.URL.customer + `?n=${this.listenNodeId}&sort_order=desc&limit=2000&page=1`,
                    this.Helper.header()
                )
                .then((res) => {
                    if (res.data.data.length > 0) {
                        ...
                        this.$store.dispatch(
                            'SET_NODE_NODE_CUSTOMER_CODE_ArrData',
                            arr.length > 0 ? arr : null
                        )
                    } else {
                        // this.openNotification('warn', null, 'Roles data is empty!', ' Please create a new role data')
                ...
                .catch((err) => {
                    this.openNotification(
                        ...
                        'Failed to get Customer Code list',
