    ...
    methods: {
        ...
        formData(form){
            ...
            if (form.formula_type === 'PERCENTAGE' && Number(form.value) > 100) {
                this.openNotification('warn', null, 'Invalid Value', 'Maksimal value 100 untuk persentase!')
                return
            }
        ...
        async updateData(){
            await axios
                .put(
                    this.URL.configuration_warning_sla + `/${this.setting_id}?n=${this.listenNodeId}`,
                ...
                .then(res => {
                    ...
                    this.closeDialog()
                    this.$emit("refresh")
                    this.openNotification("success", 'Update berhasil', 'Update konfigurasi Warning SLA berhasil')
                }).catch(err => {
                    this.loading = false
                    this.closeDialog()
                    this.$emit("refresh")
                    this.openNotification('danger', err?.response?.data?.code ?? '', 'Update gagal', err?.response?.data?.message ?? "Update konfigurasi Warning SLA gagal")
                })
        },
        async addData() {
            await axios
                ...
                .then(res => {
                    ...
                    this.closeDialog()
                    this.$emit("refresh")
                    this.openNotification("success", 'Data berhasil dibuat', 'Berhasil membuat konfigurasi Warning SLA')
                }).catch(err => {
                    this.loading = false
                    this.closeDialog()
                    this.$emit("refresh")
                    this.openNotification('danger', err?.response?.data?.code ?? '', 'Gagal membuat data', err?.response?.data?.message ?? "Gagal membuat data konfigurasi Warning SLA")
                })
        },
        cancel() {
            this.handleClearForm()
