...
export default {
    ...
    methods: {
        ...
        async getApplicationList() {
            ...
            try {
                const res = await axios.get(
                    `${this.URL.application_list}?n=${this.listenNodeId}`,
                    this.Helper.header()
                )
                ...
                if (res.data.data.length > 0) {
                    ...
                    this.openNotification(
                        'warn',
                        null,
                        'Application list data is empty!',
                        ' Please create a new application list'
                    )
                }
            } catch (err) {
                this.openNotification(
        ...
        async handleSubmitData() {
            this.loading = true
            try {
                const url = `${this.URL.application_role}${
                    this.app_role_id ? `/${this.app_role_id}` : ''
                }?n=${this.listenNodeId}`
                ...
                this.openNotification(
                    ...
                    null,
                    'Success',
                    res?.data?.message || this.app_role_id
                        ? 'Success Update Data'
                        : 'Success Create Data'
                )
            } catch (err) {
