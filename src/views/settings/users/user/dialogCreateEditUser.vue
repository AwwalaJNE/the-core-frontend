    ...
    methods: {
        getDataDetail(val) {
            if (val.app_role.length > 0) {
                ...
                this.$store.dispatch(
                    'SET_USER_USER_APPLICATION_ROLE',
                    mainAppRole.role?.[0]?.app_role_id || ''
                )

                const dataInfo =
                    mainAppRole.role?.map(({ app_role_name, app_role_id }) => ({
                        label: app_role_name,
                        value: app_role_id,
                    })) || []
                ...
                if (otherAppRoles.length) {
                    const template =
                        this.$store.getters.getInputs.user
                            .dynamicinputcomponent_user_other_application_role.inputs

                    const arr = otherAppRoles.map(({ app, role }) => ({
                        inputs: template.map((field) => ({
                            ...field,
                            value:
                                field.key === 'helper_dynamic_user_application_name'
                                    ? app
                    ...
                    this.$store.dispatch(
                        'SET_USER_DYNAMICINPUTCOMPONENT_USER_OTHER_APPLICATION_ROLE',
                        arr
                    )
                    ...
                    this.$store.dispatch(
                        'SET_USER_HELPER_DYNAMIC_USER_APPLICATION_ROLE_ArrData',
                        otherDataInfo
                    )
                }
            }
        ...
        formData(form) {
            ...
            if (
                Array.isArray(form.dynamicinputcomponent_user_other_application_role) &&
                form.dynamicinputcomponent_user_other_application_role.length
            ) {
                formPayload['app_role'].push(
                    ...form.dynamicinputcomponent_user_other_application_role
                        .map((item) => ({
                            app: item.inputs?.[0]?.value || '',
                            role: Array.isArray(item.inputs?.[1]?.value)
                                ? item.inputs[1].value
                                : [item.inputs?.[1]?.value],
                        }))
        ...
        onChangeCustom(type, val, obj, obj2, helper) {
            switch (type) {
                case 'dynamicinputcomponent_user_other_application_role':
                    switch (obj?.typeInput) {
                        ...
                        case 'select|hidden':
                            ...
                            let latest_data =
                                this.$store.getters.getInputs.user
                                    .dynamicinputcomponent_user_other_application_role.arrData
        ...
        inputFocus(obj, val, info) {
            if (obj.key === 'user_node_id' || obj.key.includes('user_additional_node_id')) {
                ...
                let app_role_name_index = Number.isInteger(parsedIndex)
                    ? this.$store.getters.getInputs.user
                          .dynamicinputcomponent_user_other_application_role.arrData?.[parsedIndex]
                          ?.inputs[0]?.value
                    : this.$store.getters.getInputs.user.user_application_name.value

                this.autoComplateUrl =
                    this.URL.application_role_list +
                    '?n=' +
                    this.listenNodeId +
                    `&sort_order=desc&limit=10&page=1&search_by=${app_role_name_index}`
        ...
        async getApplicationList() {
            ...
            try {
                ...
                if (res.data.data.length > 0) {
                    ...
                    this.$store.dispatch(
                        'SET_USER_HELPER_DYNAMIC_USER_APPLICATION_NAME_ArrData',
                        arr
                    )
                } else {
