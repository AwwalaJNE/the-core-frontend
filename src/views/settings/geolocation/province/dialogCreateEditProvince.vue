        ...
        listenDataItem() {
            ...
            if (obj != undefined) {
                obj.hasOwnProperty('geolocation_province_time_zone')
                    ? (obj['geolocation_province_time_zone'] = obj['geolocation_province_time_zone']
                          .replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, '_')
                          .toLowerCase())
            ...
            await axios
                ...
                .then((res) => {
                    if (res.data.data.length > 0) {
                        ...
                        this.$store.dispatch(
                            'SET_GEOLOCATION_PROVINCE_GEOLOCATION_COUNTRY_ID_ArrData',
            ...
            await axios
                ...
                .then((res) => {
                    if (res.data.data.length > 0) {
                        ...
                        this.$store.dispatch(
                            'SET_GEOLOCATION_PROVINCE_GEOLOCATION_PROVINCE_TIME_ZONE_ArrData',
                            arr.length > 0 ? arr : null
                        )
                    } else {
                        // this.openNotification('warn', null, 'Roles data is empty!', ' Please create a new role data')
            ...
            await axios
                .put(
                    this.URL.geolocation_province +
                        `/${this.geolocation_province_id}?n=${this.listenNodeId}`,
