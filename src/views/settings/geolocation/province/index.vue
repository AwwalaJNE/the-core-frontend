<template>
    <div>
        ...
        <dialog-confirm
            title="Remove Geolocation Province"
            :message="`Are you sure you want to remove this geolocation province with id ${this.id}?`"
...
export default {
    ...
    methods: {
        ...
        actionRemove(val){
            this.id = val.geolocation_province_id;
        ...
        async removeData(){
            await axios
                .delete(
                    this.URL.geolocation_province + `/${this.id}?n=${this.listenNodeId}`,
                    this.Helper.header())
                .then(res => {
                    ...
                    this.closeDialogConfirmRemove()
                    this.loading = false
                    this.openNotification('danger', err.response ? err.response.data.code : '', 'Romove role is failed', err.response.data.message)
                })
        },
