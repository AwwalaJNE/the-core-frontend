                ...
                <div>
                    ...
                    <template v-if="!is_edit">
                        <template v-if="vehicle.length === 0">
                            ...
                            <img
                                src="@/assets/svg/defaultVehicle.svg"
                                alt="Core JNE Default Vehicle"
                    ...
                    <template v-else>
                        ...
                        <vs-row v-for="(item, index) in vehicle" :key="index">
                            <vs-col w="12" v-if="item.is_active">
                                <vehicle-card :data="item" :isActive="item.is_active" />
                            </vs-col>
                        </vs-row>
                    </template>
                </div>
...
export default {
    ...
    methods: {
        async getDataDetail(val) {
            ...
            this.$store.dispatch(
                'SET_SURAT_MUATAN_STOCK_NODE_ID_ORIGIN_ValueData',
                val.node_id_origin.toString()
            )
            this.$store.dispatch(
                'SET_SURAT_MUATAN_STOCK_NODE_ID_DESTINATION_ValueData',
                val.node_id_destination.toString()
            )
        ...
        async getManifestVehicle() {
            ...
            try {
                const res = await axios.get(
                    `${this.URL.manifest_vehicle}/${this.edit_data.manifest_number}?n=${this.listenNodeId}`,
                    this.Helper.header()
                )
        ...
        querySearch(queryString, cb) {
            axios
                .get(
                    this.URL.branch_list_v2 + `?n=${this.listenNodeId}&s=${queryString}`,
                    this.Helper.header()
                )
                .then((res) => {
        ...
        formData(form) {
            ...
            if (!this.is_edit) {
                formWithoutId.vehicle = this.vehicle_form
                    ?.map((item) => item.state)
                    ?.map((item) => ({
            ...
            formWithoutId.schedule_id =
                this.vehicle.find((item) => item?.state?.is_active)?.state?.shipment_schedule_id ||
                this.edit_data?.schedule_id ||
                null // TODO: CONFIRM AGAIN
        ...
        async handleSubmitData() {
            ...
            try {
                const res = this.id
                    ? await axios.put(
                          `${this.URL.sm_stock}/${this.id}?n=${this.listenNodeId}`,
                          this.form,
                          this.Helper.header()
                      )
                    : await axios.post(
                          `${this.URL.sm_stock}?n=${this.listenNodeId}`,
                          this.form,
                          this.Helper.header()
                      )
                this.openNotification(
                    ...
                    null,
                    'Success',
                    res?.data?.message || this.id ? 'Success Update Data' : 'Success Create Data'
                )
            ...
            if (!this.is_edit) {
                ...
                let created_vehicle = {
                    ...
                    state: {
                        ...
                        origin_vehicle_tlc:
                            form?.origin_vehicle?.value || form.origin_vehicle || '',
        ...
        updateVehicleValueBySchedule(vehicle, vehicle_form) {
            ...
            if (this.selected_manifest_vehicle === '')
                this.selected_manifest_vehicle = this.vehicle.find(
                    (item) => item.state.is_active === true
                )?.key
        },
    },
    mounted() {
