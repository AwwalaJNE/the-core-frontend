...
export default {
    ...
    methods: {
                    ...
                    if (form.hasOwnProperty(relatedKey)) {
                        switch (radioValue) {
                            case 'hari':
                                ...
                                this.$store.dispatch("SET_SLA_NODE_TO_NODE_A_" + relatedKey.toUpperCase() + "_ArrValueData", 'menit');
                                break;
                            case 'jam':
                                form[relatedKey] *= 60;
                                this.$store.dispatch("SET_SLA_NODE_TO_NODE_A_" + relatedKey.toUpperCase() + "_ArrValueData", 'menit');
        ...
        async getDataType(){
            ...
            this.$store.dispatch("SET_SLA_NODE_TO_NODE_A_SERVICE_CODE_ArrData", typeArray)
            ...
            //             })
            //             this.typeArray = arr
            //             this.$store.dispatch("SET_SLA_NODE_TO_NODE_A_SERVICE_CODE_ArrData", arr)
            //         } else {
        ...
        async updateData(){
            ...
            if (isActiveDifferent && areOthersEqual) {
                await axios
                    .patch(
                        this.URL.sla_node_to_node_a + `/${this.sla_id}?n=${this.listenNodeId}`,
                        JSON.stringify({
        ...
        async addData() {
            await axios
                .post(
                    this.URL.sla_node_to_node_a + `?n=${this.listenNodeId}`,
                    JSON.stringify(this.form), 
                    this.Helper.header())
                .then(res => {
                    this.handleClearForm()
