...
export default {
    ...
    methods: {
        async getDataDetail(val) {
            ...
            this.$store.dispatch(
                'SET_RUNSHEET_LIMIT_REFERENCE_VALUE_ValueData',
                val.reference_value
            )
            this.$store.dispatch(
                'SET_RUNSHEET_LIMIT_REFERENCE_VALUE_ArrData',
        ...
        async handleSubmitData() {
            this.loading = true
            try {
                const res = this.runsheet_limit_id
                    ? await axios.put(
                          `${this.URL.runsheet_limit_setting}/${this.runsheet_limit_id}?n=${this.listenNodeId}`,
                          this.form,
                          this.Helper.header()
                      )
                    : await axios.post(
                          `${this.URL.runsheet_limit_setting}?n=${this.listenNodeId}`,
                          this.form,
                          this.Helper.header()
                      )
                this.openNotification(
                    ...
                    null,
                    'Success',
                    res?.data?.message || this.runsheet_limit_id
                        ? 'Success Update Data'
                        : 'Success Create Data'
                )
            } catch (err) {
