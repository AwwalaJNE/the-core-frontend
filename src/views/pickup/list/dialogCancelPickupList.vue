        ...
        <template v-slot:content>
            <div>
                <form-input-controller 
                    ref="formPickupListCancelController"
                    @formData="formData"
                    :dataItem="listenDataItem"
                    typeForm="pickup_list_cancel"
                />
            </div>
        </template>
...
export default {
    ...
    methods: {
        handleSubmit(){
            this.$refs.formPickupListCancelController.handleSubmit() // trigger function submit form dari luar component formInputController
        },
        formData(form){
            this.form = form
            this.loading = true
        ...
        handleClearForm(){
            this.$refs.formPickupListCancelController.handleClearForm()
            this.pickup_number=''
            this.form = {}
        },
        async updateData() {
            await axios
                ...
                .then(res => {
                    ...
                    this.loading = false
                    this.$emit("refresh")
                    this.openNotification(null, 'Success', 'Cancel pickup list is success')
                }).catch(err => {
                    this.loading = false
                    this.handleClearForm()
                    this.closeDialog()
          ...
          await axios
              ...
              .then(res => {
                if(res.data.data.length > 0) {
                  ...
                  this.$store.dispatch("SET_PICKUP_LIST_CANCEL_STATUS_ArrData", arr.length > 0 ? arr : null)
                }
