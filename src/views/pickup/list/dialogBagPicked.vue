    ...
    methods: {
        async handlePickupData(val) {
            if (
                val !== null &&
                val !== undefined &&
                val.pickup_detail !== null &&
                val.pickup_detail !== undefined
            ) {
        ...
        async getDataBagDetail(bagId) {
            ...
            try {
                ...
                let data = res.data
                let bag_des =
                    data.data && data.data.destination ? data.data.destination.node_code : '-'
                let arr = data.detail

                arr.forEach((item, index) => {
                    item['no'] = index + 1
                    item['destination_code'] = item.destination_code
        ...
        scanBag() {
            let dataFoundFromList = this.bagNumberList.some((item) => item.value == this.scan_bag)
            ...
            if (dataFoundFromList) {
                ...
                this.openNotification(
                    'danger',
                    err.response ? err.response.data.code : '',
                    'Select item is failed',
                    'Bag or Connote not found!'
                )
            }
            this.scan_bag = null
        },
