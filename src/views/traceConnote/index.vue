        ...
        <section>
            <vs-row justify="space-around">
                <vs-col>
                    <div class="box view">
                        <form
                            @submit.prevent="processConnoteNumber"
                            style="display: flex; margin-top: 2em"
                        >
            ...
            <vs-row justify="space-around">
                ...
                >
                    ...
                    <div
                        class="box view"
                        v-else-if="
                            (koli_number && !connote_found && !loading) ||
                            (koli_number !== '' && !connote_found && !loading)
                        "
                    >
                        <div style="margin-top: 2.5em">connote tidak ditemukan</div>
                    </div>
                </vs-col>
            </vs-row>
        </section>
            ...
            await axios
                ...
                .then((res) => {
                    if (res.data.data && Object.keys(res.data.data).length > 0) {
                        ...
                        const filteredConnote = this.findConnoteByNumber(
                            multiKolis,
                            this.koli_number
                        )
                        if (filteredConnote == undefined) {
                            const koliTrimed = this.koli_number.slice(0, -2)
                            const filteredConnoteAfterTrimed = this.findConnoteByNumber(
                                multiKolis,
                                koliTrimed
                            )
                            this.selectedConnote =
                                filteredConnoteAfterTrimed == undefined
                                    ? null
                                    : filteredConnoteAfterTrimed.connote_number
                        } else {
                        ...
                        dataInformation = [
                            ...
                            },
                            {
                                key: 'Amount COD',
                                value:
                                    'Rp ' + Intl.NumberFormat('en-GB').format(response.amount_cod),
                                width: 6,
                            },
                            {
                                key: 'Packing Kayu',
