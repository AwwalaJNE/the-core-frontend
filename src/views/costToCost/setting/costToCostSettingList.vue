<template>
    <div>
        ...
        <table-master
            hideColumnKey="cost-to-cost-setting"
            :dataTable="dataTable"
...
export default {
    ...
    methods: {
        ...
        closeDialogConfirm() {
            ...
            this.$store.dispatch(
                'SET_COST_TO_COST_SETTING_DYNAMICINPUTCOMPONENT_COST_TO_COST_RULE',
                {}
            )
            this.$store.dispatch(
                'SET_COST_TO_COST_SETTING_DYNAMICINPUTCOMPONENT_COST_TO_COST_DETAIL_VALUE',
                {}
            )
        },
        ...
        actionDetail(row) {
            this.$router.push({
                ...
                params: { inbound_number: row.inbound_number },
            })
        },
        ...
        actionUpdate(val) {
            if (this.dataTable.length > 0) {
                ...
                this.$store.dispatch(
                    `SET_COST_TO_COST_SETTING_COST_OWNER_NODE_ID_ValueData`,
                    val['cost_owner'][0]
                ) // asumsi ada flag node_name (samain dg querysearch. klo mau dinamis pakein prop aja)
                this.$store.dispatch(
                    `SET_COST_TO_COST_SETTING_COST_PAYER_NODE_ID_ValueData`,
                    val['cost_payer'][0]
                ) // asumsi ada flag node_name
