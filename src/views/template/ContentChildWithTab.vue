...
export default {
    ...
    methods: {
        buildTabs() {
            ...
            const parentRecord =
                matched.length > 1 && matched[matched.length - 2]?.path !== ''
                    ? matched[matched.length - 2]
                    : matched[matched.length - 1]

            const parentRoute = this.findRouteByName(
            ...
            this.childrenData = (parentRoute.children || [])
                .filter((child) => {
                    ...
                    if (typeof perm === 'string')
                        return this.listenPermissions?.core?.includes(perm)
                    if (Array.isArray(perm))
                        return perm.some((p) => this.listenPermissions?.core?.includes(p))
                    return false
                })
                .map((child) => {
            ...
            if (
                ...
                this.$route.path.replace(/\/+$/, '') === `/${parentPath}`
            ) {
