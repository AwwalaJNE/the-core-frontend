<template>
    <dialog-master
        ...
        width="md"
    >
        <template v-slot:header> Add Remark </template>
                ...
                <vs-col xs="12" sm="12" lg="12">
                    <template v-if="gudang_arr.length === 0">
                        <p class="no-warehouse">
                            <b>Notes</b> : Status Hold WH tidak tersedia karena tidak terdapat
                            gudang pada lokasi Anda
                        </p>
                    </template>
                </vs-col>
            </vs-row>
...
export default {
    ...
    methods: {
        ...
        async getDataStatus() {
            ...
            await axios
                .get(
                    this.URL.tracing_status +
                        `?n=${this.listenNodeId}&sort_order=desc&limit=2000&page=1&item_number=${this.itemNumber}`,
                    this.Helper.header()
                )
                .then((res) => {
            ...
            await axios
                ...
                .then((res) => {
                    ...
                    this.openNotification(
                        ...
                        null,
                        'Success',
                        res?.data?.message ?? 'Create new tracing remark is success'
                    )
                })
                .catch((err) => {
                    this.loadingRemark = false
                    this.openNotification(
                        'danger',
                        err?.response?.data?.code ?? '',
                        'Create new tracing remark failed',
                        err?.response?.data?.message ?? 'Something went wrong'
                    )
                })
