<template>
    <dialog-master :actived="listenActive" width="lg" :closeDialog="cancel" class="custom-width">
...
export default {
        ...
        active: function (val) {
            if (val == true) {
                ...
                if (Object.keys(this.listenDataItem).length > 0) {
                    ...
                    setTimeout(() => {
                        const vehicleModes =
                            this.$store.getters['getInputs']['surat_muatan']['manifest_method_id'][
                                'arrData'
                            ]
                        if (vehicleModes && vehicleModes.length > 0) {
                            const selectedMode = vehicleModes.find(
                                (mode) => mode.value === this.listenDataItem.manifest_method_id
                            )
                            if (
                                selectedMode &&
                                selectedMode.data &&
                                selectedMode.data.vehicle_prefix
                            ) {
                                console.log(
                                    'Setting initial prefix:',
                                    selectedMode.data.vehicle_prefix
                                )
                                this.$store.dispatch(
    ...
    methods: {
        initDataItem() {
            ...
            this.vehicle_mode_id = this.listenDataItem['vehicle_mode_id']
            ...
            if (
                this.listenDataItem.manifest_number &&
                this.listenDataItem.manifest_number.includes('-')
            ) {
                ...
                if (parts.length === 2) {
                    ...
                    this.$store.dispatch(
                        'SET_SURAT_MUATAN_MANIFEST_NUMBER',
                        this.listenDataItem.manifest_number
                    )
                }
            } else {
                ...
                this.listenDataItem['detail'].map((data) => {
                    ...
                    if (data.item_number) {
                        ...
                        if (
                            ...
                            data['bag']['destination']['node_tariff_code']
                        ) {
            ...
            if (info && info.hasOwnProperty('key')) {
                ...
                switch (info['key']) {
                    ...
                    case 'node_id_destination':
                        let transit =
                            this.$store.getters['getInputs']['surat_muatan'][
        ...
        formData(form) {
            let node_id = form['node_id_origin']
            ...
            if (form.hasOwnProperty('dynamicinputcomponent_node_id_transit')) {
                if (form['dynamicinputcomponent_node_id_transit'].length > 0) {
                    form['node_id_transit_1'] = form['dynamicinputcomponent_node_id_transit'][0]
                        ? form['dynamicinputcomponent_node_id_transit'][0]['inputs'][0]['data'][
                              'node_id'
                          ]
                        : ''

                    form['node_id_transit_2'] = form['dynamicinputcomponent_node_id_transit'][1]
                        ? form['dynamicinputcomponent_node_id_transit'][1]['inputs'][0]['data'][
                              'node_id'
                          ]
                        : ''

                    form['node_id_transit_3'] = form['dynamicinputcomponent_node_id_transit'][2]
                        ? form['dynamicinputcomponent_node_id_transit'][2]['inputs'][0]['data'][
                              'node_id'
                          ]
                        : ''
                }
        ...
        updateValue(val) {
            if (this.dataItem !== undefined) {
                if (this.dataItem.status !== 'READY' || this.dataItem.is_orion == '1') {
                    let notification =
                        this.dataItem.status === 'CANCELED' ? 'SM is Canceled' : 'SM is Departed'
                    if (this.dataItem.is_orion == '1') {
                        notification = 'ORION DATA'
                    }
                    this.openNotification('warning', 'Edit Forbidden', notification)
                    return
                }
            }
