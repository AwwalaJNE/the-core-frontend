    ...
    >
        ...
        <template v-slot:content>
            <vs-col xs="12" sm="12" lg="12">
                <nav-item v-model="navActive" :navItem="navItem" @activeTab="activeTab" />
            </vs-col>
            <vs-col xs="12" sm="12" lg="12">
                <template v-if="navActive === 'k-MANAGE'">
                    <template v-if="list_manifest_vehicle.length > 0">
                        <p class="notes">
                            <b>Notes</b>: *Click a card to select a vehicle — changes save
                            automatically.
                        </p>
                        <radio
                    ...
                    <template v-else>
                        <p>
                            No vehicle data is currently available. Please create a new vehicle
                            first.
                        </p>
                    </template>
                </template>
                    ...
                    <template v-if="hasFlightNumber">
                        <div style="position: absolute; right: 20px; top: 15px">
                            <span
                                ...
                                @click="clearInput"
                                :data-testid="`remove-flight-number-button`"
                            >
                        ...
                        <template v-if="!loading">
                            <template v-if="!is_found">
                                <p>
                                    Flight number not found. Kindly enter a valid code or create the
                                    record manually.
                                </p>
                            </template>
                            <template v-else>
        ...
        <template
            v-slot:footer
            v-if="
                navActive === 'k-NEW-AUTO' ||
                navActive === 'k-NEW-MANUAL' ||
                navActive === 'k-NEW-SCHEDULE'
            "
        >
    ...
    methods: {
        ...
        setDialogActive() {
            switch (this.listenManifestMethod) {
                case 1:
                    ...
                    this.$store.dispatch(
                        'SET_SURAT_MUATAN_VEHICLE_EMPLOYEE_DRIVER_ID_visible',
                        false
                    )
                    this.$store.dispatch(
                        'SET_SURAT_MUATAN_VEHICLE_ORIGIN_BRANCH_CODE_visible',
                        true
                    )
                    this.$store.dispatch(
                        'SET_SURAT_MUATAN_VEHICLE_DESTINATION_BRANCH_CODE_visible',
                        true
                    )
                    break
                case 2:
        ...
        formData(form) {
            if (this.submitType === 'api') {
                ...
                form.destination_branch_code =
                    form.destination_branch_code?.value || form.destination_branch_code
                form.vehicle_id =
                    this.vehicle_id || form?.vehicle_id?.vehicle_id || form?.vehicle_id
                ...
                if (this.navActive === 'k-NEW-AUTO') {
                    ...
                    data = {
                        origin_vehicle:
                            form.origin_branch_code?.value ||
                            form?.origin_branch_code?.branch_code?.slice(0, 3),
                        origin_vehicle_name:
                            form.origin_branch_code?.label || form?.origin_branch_code?.node_name,
                        destination_vehicle:
                            form.destination_branch_code?.value ||
                            form?.destination_branch_code?.branch_code?.slice(0, 3),
                        destination_vehicle_name:
                            form.destination_branch_code?.label ||
                            form?.destination_branch_code?.node_name,
                ...
                if (data.etd_vehicle > data.eta_vehicle) {
                    this.openNotification(
                        ...
                        '',
                        'Failed',
                        'ETD tidak boleh lebih besar dari ETA'
                    )
                    return
                }
        ...
        async getVehicle(query) {
            ...
            try {
                const res = await axios.get(
                    `${this.URL.vehicle}?n=${this.listenNodeId}&search_by=vehicle_police_no&s=${query}`,
                    this.Helper.header()
                )
        ...
        async getManifestVehicle() {
            ...
            try {
                const res = await axios.get(
                    `${this.URL.manifest_vehicle}/${this.listenManifestNumber}?n=${this.listenNodeId}`,
                    this.Helper.header()
                )
                ...
                this.selected_manifest_vehicle =
                    arr.find((item) => item.status === 'ACTIVE')?.manifest_vehicle_log_id || null
        ...
        async chooseManifestVehicle(log_id) {
            ...
            try {
                const res = await axios.patch(
                    `${this.URL.manifest_vehicle}/${log_id}/active?n=${this.listenNodeId}`,
                    null,
                    this.Helper.header()
                )
                this.openNotification('success', null, 'Success', 'Update manifest vehicle success')
        ...
        async removeManifestVehicle(log_id) {
            ...
            try {
                const res = await axios.delete(
                    `${this.URL.manifest_vehicle}/${log_id}?n=${this.listenNodeId}`,
                    this.Helper.header()
                )
        ...
        async updateValue(newKey, done) {
            ...
            try {
                const res = await axios.patch(
                    `${this.URL.manifest_vehicle}/${newKey}/active?n=${this.listenNodeId}`,
                    null,
                    this.Helper.header()
                )
                this.openNotification('success', null, 'Success', 'Update manifest vehicle success')
        ...
        async processFlightNumber() {
            ...
            try {
                ...
                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_FLIGHT_SCHEDULE',
                    this.formatTimezone(
                        data?.detailJson?.timingInformation?.departure?.runway?.scheduled?.iso
                    )
                )
                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_VEHICLE_ID',
                    data?.detailJson?.flightSummary?.airline?.shortName
                )
                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_ORIGIN_BRANCH_CODE',
                    data?.detailJson?.routeInformation?.departure?.airport?.name
                )
                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_DESTINATION_BRANCH_CODE',
                    data?.detailJson?.routeInformation?.arrival?.airport?.name
                )
                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_ETD',
                    this.formatTimezone(
                        data?.detailJson?.timingInformation?.departure?.runway?.estimated?.iso
                    )
                )
                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_ETA',
                    this.formatTimezone(
                        data?.detailJson?.timingInformation?.arrival?.runway?.estimated?.iso
                    )
                )

                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_VEHICLE_ID_ValueData',
                    data?.detailJson?.flightSummary?.airline?.iata
                )
                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_ORIGIN_BRANCH_CODE_ValueData',
                    data?.detailJson?.routeInformation?.departure?.airport?.iata
                )
                this.$store.dispatch(
                    'SET_SURAT_MUATAN_VEHICLE_DESTINATION_BRANCH_CODE_ValueData',
                    data?.detailJson?.routeInformation?.arrival?.airport?.iata
                )
        ...
        handleClearForm() {
            if (
                (this.navActive === 'k-NEW-AUTO' || this.navActive === 'k-NEW-MANUAL') &&
                this.$refs.formSuratMuatanVehicleController
            ) {
        ...
        async getTableData(limit, page, q, from, to, searchBy) {
            ...
            try {
                const res = await axios.get(
                    `${this.URL.schedule}?n=${this.listenNodeId}&sort_order=desc&limit=${limit}&page=${page}&s=${query}&filter_date_by=etd&start_date=${startDate}&end_date=${endDate}&search_by=${searchBy}&vehicle_type=${this.listenManifestMethod}`,
                    this.Helper.header()
                )
        ...
        updateSelected(val, checkedItem) {
            ...
            if (val.shipment_schedule_id === this.selected_manifest_vehicle)
                this.selected_manifest_vehicle = ''

            if (this.selected_manifest_vehicle === '')
                this.selected_manifest_vehicle = checkedItem?.[0]?.shipment_schedule_id
            ...
            this.vehicle = checkedItem.map((item, idx) => ({
                ...
                state: {
                    ...
                    origin_vehicle_tlc:
                        this.getTLC(item?.origin_name) || item?.origin_identifier || '',
                    destination_vehicle_tlc:
                        this.getTLC(item?.destination_name) || item?.destination_identifier || '',
                    vehicle_id: item?.vehicle_name || '',
                    pic_employee_id: '',
                    ...
                    is_active:
                        item?.shipment_schedule_id === this.selected_manifest_vehicle || false,
                },
            }))
