    ...
    methods: {
        ...
        updateValue(val) {
            ...
            let hasData = this.dataTable.filter(
                (item) => item["bag_number"] == this.item_number
            );

            if (hasData.length == 0) {
                // this.getDataManifest(this.item_number);
            ...
            // if (this.manifest_number) {
            //     console.log("Ada manifest number EDIT")
            // } else {
            //     console.log("Ga Ada manifest number CREATE");
            //     this.handleSubmit();
            //     // this.createSuratMuatan();
            // }
        },
        ...
        async updateSuratMuatan() {
            this.loading = true;
            try {
                const res = await axios.put(`${this.URL.revamp_surat_muatan}/${this.manifest_number}?n=${this.listenNodeId}`, JSON.stringify(this.form), this.Helper.header());
        ...
        async addSuratMuatanDetail() {
            ...
            try {
                ...
                this.openNotification(null, "Success", "Add detail surat muatan success");
            } catch (err) {
                this.loading = false;
                this.openNotification("danger", err?.response?.data?.code ?? '', "Failed", err?.response?.data?.message ?? 'Something went wrong');
            }
