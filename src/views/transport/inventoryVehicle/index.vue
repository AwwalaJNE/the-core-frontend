                        ...
                        <template>
                            <vs-row>
                                ...
                                <vs-col vs-align="center" xs="6" sm="3" lg="3">
                                    <template v-if="DataVehicle.length > 1">
                                        <vs-select class="m-select" filter autocomplete="off" :multiple="false" placeholder="All Vehicle" v-model="vehicle_id" :border="true" @change="updateVehicle">
                                            <template v-if="DataVehicle.length > 1">
                                                <vs-option v-for="(item, key) in DataVehicle" :key="key" :label="item.label"
                                                    :value="item.value">
                                                    {{ item.label }}
                                                </vs-option>
                                            </template>
                                ...
                                <vs-col vs-align="center" xs="6" sm="3" lg="3">
                                    <template v-if="DataDriver.length > 1">
                                        <vs-select class="m-select" filter :multiple="false" placeholder="All Driver" v-model="driver_id" :border="true" @change="updateDriver">
                                            <template v-if="DataDriver.length > 1">
                                                <vs-option v-for="(item, key) in DataDriver" :key="key" :label="item.label" :value="item.value">
                                                    {{ item.label }}
                                                </vs-option>
                                            </template>
                                        </vs-select>
                                ...
                                <vs-col xs="5" sm="3" lg="3" offset="1" class="mb-15">
                                    <search-input ref="searchInput" @searchValue="searchValue" :placeholder="`Search Koli`"/>
                                </vs-col>
                            </vs-row>
                            <transition name="slide-fade">
                                <vehicle-list :ref="navActive" :query="tempSearch"
                                    :queryVehicle="vehicle_id" :queryBags="bag_number" :queryDriver="driver_id"/>
                            </transition>
                        </template>
