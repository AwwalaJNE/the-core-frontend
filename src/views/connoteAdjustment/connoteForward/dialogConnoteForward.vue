<template>
    ...
    >
        ...
        <template v-slot:content>
            <vs-row>
                ...
                <vs-col
                    ...
                    sm="6"
                    lg="6"
                    v-if="status === 'PS3' && connote_number && !isCreateManually"
                >
                    <vs-input
                        type="text"
                        autofocus
                ...
                <div v-if="!isCreateManually">
                    ...
                    <div style="display: flex; align-items: center; justify-content: space-between">
                        ...
                        <div class="change-container">
                            <span class="change-link" @click="selectCreate"> Create Manually </span>
                        </div>
                    </div>
        ...
        <template
            ...
            v-if="
                status === 'PS2' ||
                (status === 'PS3' && crisscross_number) ||
                (status === 'PS3' && isCreateManually)
            "
        >
...
export default {
    ...
    watch: {
        isEdit: function (val) {
            if (val == false) {
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_ADDRESS_TYPE_isDisabled',
                    true
                )

                this.$store.dispatch('SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_NAME_isDisabled', true)
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_PHONE_NUMBER_isDisabled',
                    true
                )
                this.$store.dispatch('SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_EMAIL_isDisabled', true)
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_STREET_ADDRESS_isDisabled',
                    true
                )
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_ADMINISTRATIVE_ADDRESS_isDisabled',
                    true
                )
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_ZIP_CODE_isDisabled',
                    true
                )
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_TARIFF_CODE_isDisabled',
                    true
                )

                this.$store.dispatch('SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_NAME_isDisabled', true)
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_PHONE_NUMBER_isDisabled',
                    true
                )
                this.$store.dispatch('SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_EMAIL_isDisabled', true)
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_STREET_ADDRESS_isDisabled',
                    true
                )
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_ADMINISTRATIVE_ADDRESS_isDisabled',
                    true
                )
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_ZIP_CODE_isDisabled',
                    true
                )
                this.$store.dispatch(
                    'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_TARIFF_CODE_isDisabled',
                    true
                )
            } else {
                this.$store.dispatch(
    ...
    methods: {
        ...
        async getCrisscross(limit, page) {
            ...
            try {
                const res = await axios.get(
                    `${this.URL.crisscross}/${this.connote_number}?n=${this.listenNodeId}&sort_order=desc&limit=${limit}&page=${page}`,
                    this.Helper.header()
                )
        ...
        async scanConnote() {
            ...
            try {
                const res =
                    this.crisscross_number && !this.isCreateManually
                        ? await axios.get(
                              `${this.URL.connote}/${this.crisscross_number}?n=${this.listenNodeId}`,
                              this.Helper.header()
                          )
                        : await axios.get(
                              `${this.URL.connote_forward}/${this.connote_number}/scan?n=${this.listenNodeId}`,
                              this.Helper.header()
                          )
                if (res.data.data) {
                    ...
                    let obj = {
                        ...
                        connote_receiver_geolocation_subdistrict_id:
                    ...
                    this.$store.dispatch(
                        'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_ADDRESS_TYPE',
                        data?.connote_receiver_address_type
                    )
                    this.dataItem = obj
                    this.connote_number = this.crisscross_number
                        ? this.connote_number
                        : res.data.data.connote_number || this.connote_number
        ...
        async formData(form) {
            ...
            this.form = {
                ...
                connote: {
                    ...formWithoutAdministrativeAddress,
                    connote_receiver_administrative_address:
                        connote_receiver_administrative_address?.geolocation_location_name ||
                        this.original_form?.connote?.connote_receiver_administrative_address ||
                        '',
                    connote_shipper_administrative_address:
                        connote_shipper_administrative_address?.geolocation_location_name ||
                        this.original_form?.connote?.connote_shipper_administrative_address ||
                        '',
        ...
        async handleSubmitData() {
            ...
            try {
                const res = await axios.post(
                    `${this.URL.connote_forward}?n=${this.listenNodeId}`,
                    this.form,
                    this.Helper.header()
                )
                if (res.status === 200) {
                    ...
                    if (amount_total_price <= 0) {
                        this.openNotification(
                            ...
                            '',
                            'Warning',
                            'Connote Forward berhasil dibuat tanpa Tariff'
                        )
                    } else {
                        this.openNotification(
                            null,
                            '',
                            'Success',
                            'Connote Forward berhasil dibuat'
                        )
                    }
                }
            } catch (err) {
        ...
        onChangeCustom(type, val, obj) {
            switch (type) {
                case 'connote_shipper_administrative_address':
                    this.$store.dispatch(
                        'SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_ADMINISTRATIVE_ADDRESS',
                        obj?.data?.geolocation_location_name
                    )
                    this.$store.dispatch(
                        'SET_CONNOTE_FORWARD_CONNOTE_SHIPPER_ZIP_CODE',
                        obj?.data?.geolocation_subdistrict_zip_code
                    )
                    break
                case 'connote_receiver_administrative_address':
                    this.$store.dispatch(
                        'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_ADMINISTRATIVE_ADDRESS',
                        obj?.data?.geolocation_location_name
                    )
                    this.$store.dispatch(
                        'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_ZIP_CODE',
                        obj?.data?.geolocation_subdistrict_zip_code
                    )
                    this.$store.dispatch(
                        'SET_CONNOTE_FORWARD_CONNOTE_RECEIVER_TARIFF_CODE',
                        obj?.data?.geolocation_subdistrict_tarif_code
                    )
                    break
                default:
            }
