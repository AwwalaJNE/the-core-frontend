<template>
    <dialog-master width="md" :actived="listenActive" :closeDialog="cancel" :loading="loading">
...
export default {
    ...
    methods: {
        ...
        async handleSubmit() {
            ...
            this.loading = true
            try {
                const res = this.request_id
                    ? await axios.put(
                          `${this.URL.connote_cancel}/${this.request_id}?n=${this.listenNodeId}`,
                          form,
                          this.Helper.header()
                      )
                    : await axios.post(
                          `${this.URL.connote_cancel}?n=${this.listenNodeId}`,
                          form,
                          this.Helper.header()
                      )
                this.openNotification(
                    ...
                    null,
                    'Success',
                    'Request cancel connote is success'
                )
            } catch (err) {
                this.openNotification(
                    'danger',
