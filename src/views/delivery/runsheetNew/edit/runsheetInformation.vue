...
export default {
    ...
    methods: {
        getWarningIcon(item) {
            return item.warning_koli_record_id
        ...
        updateValue(key, val, info, item = null) {
            ...
            const find = selected.find((item) => item.koli_number === obj.koli_number)

            if (find) {
                const index = selected.findIndex((item) => item.koli_number === obj.koli_number)
